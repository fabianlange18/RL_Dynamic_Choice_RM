...
MODEL_ORDER = ["MNL", "MMNL_5PT", "MMNL_2PT", "MMNLcont", "Probit", "MNLrefPrice", "MNLConsidSet", "NLogit"]
...
def parse_exec_log(log_path: str) -> Dict:
    ...
    # Extract MMNL Continuous estimation info.
    estimation_mmnl_cont_match = re.search(r'Estimation_MMNL_CONT time: ([\d.]+)s \| mu: ([-\d.]+), sigma: ([-\d.]+), lambda: ([\d.]+)', content)
    if estimation_mmnl_cont_match:
        results['metadata']['estimation_mmnl_cont_time'] = estimation_mmnl_cont_match.group(1)
        results['metadata']['estimation_mmnl_cont_mu'] = estimation_mmnl_cont_match.group(2)
        results['metadata']['estimation_mmnl_cont_sigma'] = estimation_mmnl_cont_match.group(3)
        results['metadata']['estimation_mmnl_cont_lambda'] = estimation_mmnl_cont_match.group(4)
        # Extract LL, AIC, BIC - try format with AIC/BIC first
        ll_aic_bic_same_line = re.search(r'Estimation_MMNL_CONT.*?final log likelihood: ([-\d.]+), AIC: ([\d.]+), BIC: ([\d.]+)', content, re.DOTALL)
        if ll_aic_bic_same_line:
            results['metadata']['estimation_mmnl_cont_ll'] = ll_aic_bic_same_line.group(1)
            results['metadata']['estimation_mmnl_cont_aic'] = ll_aic_bic_same_line.group(2)
            results['metadata']['estimation_mmnl_cont_bic'] = ll_aic_bic_same_line.group(3)
        else:
            # Try next-line format for backward compatibility
            ll_aic_bic_match = re.search(r'Estimation_MMNL_CONT.*?\n\s+LL: ([-\d.]+), AIC: ([\d.]+), BIC: ([\d.]+)', content, re.DOTALL)
            if ll_aic_bic_match:
                results['metadata']['estimation_mmnl_cont_ll'] = ll_aic_bic_match.group(1)
                results['metadata']['estimation_mmnl_cont_aic'] = ll_aic_bic_match.group(2)
                results['metadata']['estimation_mmnl_cont_bic'] = ll_aic_bic_match.group(3)
    ...
    mmnl_cont_eff_match = re.search(r'MMNL Cont efficient sets time: ([\d.]+)s \| sets: (\([^)]+\))', content)
    if mmnl_cont_eff_match:
        results['metadata']['mmnl_cont_effsets_time'] = mmnl_cont_eff_match.group(1)
...
def create_metadata_section(metadata: Dict, folder_name: str) -> str:
    ...
    mmnl_cont_effsets_count = _count_sets(metadata.get('mmnl_cont_effsets', ''))

    def fmt_seconds(value: str) -> str:
        rounded = _format_decimal(value, 2)
        return f"{rounded}s" if rounded != 'N/A' else 'N/A'

    # Create a table with all four model variants
    ...
    est_cont_aic = _format_decimal(metadata.get('estimation_mmnl_cont_aic', 'N/A'), 2)
    latex.append(f"    AIC & {est_mnl_aic} & {est_5pt_aic} & {est_2pt_aic} & {est_cont_aic} \\\\")
    ...
    est_cont_bic = _format_decimal(metadata.get('estimation_mmnl_cont_bic', 'N/A'), 2)
    ...
    dp_cont_time = fmt_seconds(metadata.get('dp_mmnl_cont_time', 'N/A'))
    latex.append(f"    DP Time & {dp_mnl_time} & {dp_5pt_time} & {dp_2pt_time} & {dp_cont_time} \\\\")
...
def create_latex_table(data: List[Dict], timestep: str) -> str:
    ...
    dp_method_order = ['DP_MNL', 'DP_MMNL_5PT', 'DP_MMNL_2PT', 'DP_MMNL_CONT']
