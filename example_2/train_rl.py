...
def train_rl(dp_pi, efficient_sets=None):
    ...
    for run_id in range(c.N_TRAIN_RUNS):
        ...
        for algorithm_name, algorithm_cls in C.RL_ALGORITHMS.items():
            ...
            model.learn(total_timesteps=C.TOTAL_TIMESTEPS, callback=eval_callback if C.LEARNING_CURVE_ENABLED else None, progress_bar=True)
            elapsed_seconds = time.perf_counter() - start_time
            run_times[algorithm_name] = elapsed_seconds
            model.save(f"{C.OUTPUT_DIR}/{algorithm_name}_model_run{run_id + 1}")
        ...
        training_times.append(run_times if C.LEARNING_CURVE_ENABLED else None)
